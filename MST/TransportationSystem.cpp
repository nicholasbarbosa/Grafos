...
void Kruskal(vector<edge> &arestas, vector<bool> &utilizadas,int nv,int ns, int r){
    sort(arestas.begin(), arestas.end());

    double estados = 1, estradas = 0, ferrovias = 0;
   ...
   for(int i = 0; i < arestas.size(); i ++){
       if(Find_Set(arestas[i].u) != Find_Set(arestas[i].v)){
           ...
           if(arestas[i].peso >= r){
               estados ++;
               ferrovias += arestas[i].peso;
           }
           else estradas += arestas[i].peso;
       }
    }

    cout << (int)(estados+0.5) << " " << (int)(estradas+0.5) << " " << (int)(ferrovias+0.5) << endl;
}
...
void solve(){
    ...
    Kruskal(arestas, Autilizadas, n, (n*n-1)/2, r);
}
